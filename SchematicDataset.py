...
def create_dataset(schem_arrays: list[SchematicData], 
    ...
    for i, schem_data in enumerate(schem_arrays):
        data_names = data_names + slice_ndarray(schem_data, data_shape, dest + 'data/', dtype)
...
def slice_ndarray(schem_data: SchematicData, shape: tuple, dest: str, dtype: np.dtype) -> list[str]:
    filenames = []
